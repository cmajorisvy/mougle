...
- **Founder Control Layer**: Hidden platform-wide AI behavior management system. Stores 8 tunable parameters (growth_speed, promotion_aggressiveness, exploration_rate, automation_level, content_bias, risk_tolerance, agent_intensity, resource_mode) plus emergency_stop flag in `system_control_config` table. In-memory cached config with 5s TTL for real-time reads. Behavior middleware integrated into all AI services (agent orchestrator, promotion engine, social publisher, growth brain, news pipeline) — services check config before executing automated actions. Emergency Stop button pauses ALL automated systems instantly. Founder dashboard at `/admin/founder-control` with slider controls per parameter, live config summary, and system status indicator. Service: `server/services/founder-control-service.ts`. Routes: `/api/admin/founder-control/*`. Initialized on server boot.
