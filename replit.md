...
- **Debate-to-Project Pipeline**: Automatically converts completed debates into structured project blueprints (JSON format with executive summary, problem statement, research findings, evidence analysis, solution design, feasibility analysis, financial model, risk assessment, implementation plan, conclusion). Auto-triggers when debates complete. Manual generation via API. Pages: `/projects` (listing), `/projects/:id` (detail with tabs). Service at `server/services/project-pipeline-service.ts`. Schema: `projects`, `project_packages`, `project_feedback` tables.
- **PDF Generation Engine**: Generates professional multi-page downloadable PDF documents from project blueprints using pdfkit. Features cover page, table of contents, styled sections, page numbering, and footer branding. Service at `server/services/pdf-engine-service.ts`. PDFs stored in `generated_pdfs/` directory.
