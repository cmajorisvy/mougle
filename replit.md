...
- **Phase Transition Monitoring System**: Detects when platform growth becomes self-sustaining. Tracks 5 weighted metrics: external user acquisition from apps (25%), creator repeat activity (20%), revenue vs AI cost ratio (25%), labs opportunity success rate (15%), app creation frequency (15%). Computes composite Transition Index (autonomy percentage) with 4 phases: Engine Building → Engagement Lock → Flywheel Ignition → Autonomous Growth. Features growth indicators per metric, trend detection, and raw stats panel. Admin dashboard at `/admin/phase-transition`. Service at `server/services/phase-transition-service.ts`. API: `GET /api/admin/transition-index`, `GET /api/admin/transition-metrics`.
- **Razorpay Marketplace Payment System**: Creator earnings with 70/30 revenue split (creator/platform) using Razorpay Route for automatic transfers. Features: creator payout account onboarding, marketplace order tracking, earnings dashboard with analytics. Schema: `creator_payout_accounts`, `marketplace_orders`, `creator_earnings`. Service at `server/services/razorpay-marketplace-service.ts`. Dashboard at `/creator-earnings`. API: `/api/razorpay/*`.
- **Creator Publisher Responsibility System**: Legal accountability framework for app publishers. Requires publisher profile (name, company, address, support email, business type) and agreement acceptance before publishing. Features: 3-step onboarding flow (profile → agreement → ready), platform infrastructure disclaimer on all apps, publisher identity display, IP-logged agreement acceptance. Service at `server/services/publisher-responsibility-service.ts`. Page at `/publisher`. API: `/api/publisher/*`. Schema: `creator_publisher_profiles`.
- **Creator Verification & Responsible Promotion System**: Ensures safe marketplace participation. Features: 3-tier trust levels (Explorer → Verified Creator → Trusted Publisher) with progressive capability unlocking, Responsible Promotion Declaration questionnaire (10 marketing methods, 12 promotion channels), anti-spam/illegal promotion agreement, data collection privacy notice, trust level upgrade flow. Service at `server/services/creator-verification-service.ts`. Page at `/creator-verification`. API: `/api/creator-verification/*`. Schema: `creator_promotion_declarations`, `trustLevel` column on `creator_publisher_profiles`.
- **Legal Safety Stack**: Comprehensive legal protection architecture for creator economy. Features: (1) Auto-generated risk-based disclaimers for 9 industry categories (healthcare/critical, finance/high, legal/high, education/medium, ecommerce/medium, real-estate/medium, ai-automation/high, social/medium, general/low) with regulatory tags (HIPAA, PCI-DSS, FERPA, DPDP, etc.), (2) AI Usage Policy enforcement with 6 content filter rules across healthcare, legal, finance, impersonation, harmful, privacy categories, (3) App moderation and reporting system with 10 report categories and resolve/dismiss/suspend workflow, (4) Daily creation limits by tier (free: 2 apps/5 builds, pro: 10/25, creator: 25/50), (5) Publisher identity display on app store cards with verified badges, (6) Platform infrastructure disclaimer auto-attached to all apps, (7) Publish gate checks (profile + agreement + no pending reports). Admin dashboard at `/admin/legal-safety`. Service at `server/services/legal-safety-service.ts`. Schema: `app_moderation_reports`, `app_risk_disclaimers`, `ai_usage_violations`, `daily_creation_limits`.
